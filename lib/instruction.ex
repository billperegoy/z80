defmodule Instruction do
  ...
  # NOP - 0x00
  def decode(address, <<0x00::8>>), do: IO.puts("#{address} NOP")

  # RET - 0xc9
  def decode(address, <<0xC9::8>>), do: IO.puts("#{address} RET")

  # DI - 0xf3
  def decode(address, <<0xF3::8>>), do: IO.puts("#{address} DI")

  # RST - 0xff
  def decode(address, <<0xFF::8>>), do: IO.puts("#{address} RST")

  def decode(address, <<0xE3::8>>), do: IO.puts("#{address} EX (SP), HL")
